...
if (typeof ARIA2=="undefined"||!ARIA2) var ARIA2=(function(){
  ...
  return {
    init: function(path, onready) {
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "网络连接错误，您需要刷新这个页面重新使用");
          ws_callback = {};
        };
    ...
    restart_task: function(gids) {
      ...
      if (uris.length == 0) {
        main_alert("alert-error", "文件未发现！ (BT任务不能重新开始)", 2000);
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "移除所有已完成/错误的任务", 2000);
        }
      );
    },
      ...
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          if (!result.result) {
            ...
            main_alert("alert-success", "设置正确！", 2000);
          }
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "任务列表有更新，点击“刷新”按钮进行更新");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
